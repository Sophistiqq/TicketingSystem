...
export const csat = new Elysia({ prefix: "/csat" })
  ...
  .post("/", async ({ body, user, status }) => {
    ...
    let rating = Math.round((body.rating_speed + body.rating_attitude + body.rating_impact) / 3);
  ...
  .get("/reports/:agentId", async ({ params, query, status }) => {
  ...
  .get("/my/agent", async ({ user, query, status }) => {
