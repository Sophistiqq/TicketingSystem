...
export const notifications = new Elysia({ prefix: "/notifications" })
  ...
  .get("/", async ({ user, roles, query, status }) => {
  ...
  // Get unread count
  .get("/unread-count", async ({ user, roles, status }) => {
    const typeExclusions: string[] = ["message_received"];
    if (roles?.includes("approver")) {
      typeExclusions.push("comment_added");
    }
  ...
  .post("/push-subscribe", async ({ body, user, status }) => {
  ...
  .put("/:id/read", async ({ params, user, status }) => {
