<script lang="ts">
  ...
  import { getAffectedSystems, getRequestTypes } from "../stores/reference.svelte";
    ...
    <div class="card-body gap-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <fieldset class="fieldset">
          ...
          <select id="ticket-priority" class="select select-bordered w-full" bind:value={priority} disabled={loading}>
        ...
        <fieldset class="fieldset">
          <label class="label font-medium" for="ticket-due">Due Date (optional)</label>
          <input
            id="ticket-due"
            type="datetime-local"
            class="input input-bordered w-full"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <fieldset class="fieldset">
          ...
          <select id="ticket-type" class="select select-bordered w-full" bind:value={request_type_id} disabled={loading}>
        ...
        <fieldset class="fieldset">
          <label class="label font-medium" for="ticket-system">Affected System</label>
          <select id="ticket-system" class="select select-bordered w-full" bind:value={affected_system_id} disabled={loading}>
            <option value={undefined}>— Select —</option>
            {#each systems as sys}
      ...
      <label class="label cursor-pointer justify-start gap-3">
        <input type="checkbox" class="checkbox checkbox-primary" bind:checked={requires_approval} disabled={loading} />
        <span>Requires approval before work begins</span>
      </label>
